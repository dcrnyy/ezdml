...
{$IFNDEF LINUX}
uses
  WinTypes, Classes, SysUtils, FileCtrl;
{$ELSE}
uses
  Libc, Classes, SysUtils, SyncObjs, FileCtrl;
{$ENDIF}
...
const // SessionPool
  ...
  OCI_CPOOL                  = $200;      // Attach using server handle from pool
  // OCISessionPoolCreate Modes
  OCI_SPC_REINITIALIZE       = 1;         // Reinitialize the session pool
  OCI_SPC_HOMOGENEOUS        = 2;         // Session pool is homogeneneous
  OCI_SPC_STMTCACHE          = 4;         // Session pool has stmt cache
  // OCISessionGet Modes
  OCI_SESSGET_SPOOL          = 1;         // SessionGet called in SPOOL mode
  OCI_SESSGET_CPOOL          = OCI_CPOOL; // SessionGet called in CPOOL mode
  OCI_SESSGET_STMTCACHE      = 4;         // Use statement cache
  OCI_SESSGET_CREDPROXY      = 8;         // SessionGet called in proxy mode
  OCI_SESSGET_CREDEXT        = 16;
  OCI_SESSGET_SPOOL_MATCHANY = 32;
  // ATTR Values for Session Pool
  OCI_SPOOL_ATTRVAL_WAIT     = 0;         // block till you get a session
  OCI_SPOOL_ATTRVAL_NOWAIT   = 1;         // error out if no session avaliable
  OCI_SPOOL_ATTRVAL_FORCEGET = 2;         // get session even if max is exceeded
  // OCISessionRelease Modes
  OCI_SESSRLS_DROPSESS       = 1;         // Drop the Session
  OCI_SESSRLS_RETAG          = 2;         // Retag the session

  // OCISessionPoolDestroy Modes
  OCI_SPD_FORCE              = 1;         // Force the sessions to terminate.
                                          // Even if there are some busy
                                          // sessions close them

const // Statement cache
...
type // Handle Definitions
  ...
  OCISPool           = pointer;  // OCI Session Pool
  OCIAuthInfo        = pointer;  // OCI Authentication Info
  OCIDateTime        = pointer;  // OCI Datetime
  ...
  OCITypeByName:
   ...
   OCIDateTimeFromText:
     function(envhp: OCIEnv;
              ...
              lang_name: PChar;
              lang_length: ub4;
              date: OCIDate): sword; cdecl;
   OCIDateTimeAssign:
     function(envhp: OCIEnv;
              err: OCIError;
              from: OCIDateTime;
              to_: OCIDateTime): sword; cdecl;
   OCIRawPtr:
     function(envhp: OCIEnv;
...
// SessionPool
   ...
   OCISessionGet:
     function(envhp: OCIEnv;
              ...
              tagInfo: PChar;
              tagInfo_len: ub4;
              var retTagInfo: PChar;
              var retTagInfo_len: ub4;
              var found: longbool;
              mode: ub4): sword; cdecl;
   OCISessionRelease:
     function(svchp: OCISvcCtx;
...
// Timestamps
   ...
   OCIDateTimeConstruct:
     function(envhp: OCIEnv;
              ...
              fsec: ub4;
              timezone: PChar;
              timezone_length: ub4): sword; cdecl;
   OCIDateTimeGetTimeZoneOffset:
     function(envhp: OCIEnv;
              errhp: OCIError;
              datetime: OCIDateTime;
              var hour: sb1;
              var minute: sb1): sword; cdecl;
...
var
  ...
  AllowAdjustPath: Boolean = True;       // Allow adjusting of path environment
  UseSingleEnvironment: Boolean = False; // Use a single environment for all sessions
  SingleEnv: OCIEnv = nil;               // Single global environment
{$IFDEF LINUX}
  HDLL: THandle = nil;
  HMTS: THandle = nil;
{$ELSE}
...
{$ENDIF}
  ...
  OCI90Detected:   Boolean = False;
  OCI92Detected:   Boolean = False;
  ...
  OracleBinDir:    Boolean = True;     // ORACLE_HOME bin directory exists
  ...
  StripDefaultDomain: Boolean = False;
...
function NLS_LANG: string;
function UTF8Client: Boolean;
...
var
  ...
  OracleNLSLANG: string = '';                // NLS_LANG value
  OracleUTF8Client: Boolean;                 // UTF8 Client?
  OracleDefaultDomain: string = '';          // Default Domain (from sqlnet.ora file)
...
begin
  Result := '';  
  if GetEnvironmentVariable('_NS_ORA_INSTANT_CLIENT') = 'True' then //Add by XJG
    Exit; //Add by XJG
...
begin
  ...
  begin
    ...
    Repeat
      BufSize := 1024;
      SetLength(S, BufSize);
      R := RegEnumKeyEx(Handle, Index, @S[1], BufSize, nil, nil, nil, nil);
...
begin
  ...
  if (HomeDir = '') or (not DirectoryExists(HomeDir + '\bin')) then Result := '';
...
var Path, S: string;
    ...
    i, p: Integer;
begin
  if OracleDefaultDomain = '' then
  begin
    Path := ExtractFilePath(TNSNames) + 'sqlnet.ora';
    ...
    try
      ...
      begin
        ...
        if p > 0 then
        begin
          S := Lowercase(Trim(Copy(S, 1, p - 1)));
          if S = 'names.default_domain' then
          begin
            OracleDefaultDomain := Trim(Copy(T[i], p + 1, Length(T[i])));
            Break;
          end;
        end;
      end;
  ...
  Result := OracleDefaultDomain;
end;

procedure FindAliases(Path: string);
var T: TStringList;
    i, l, p, Level, ix: Integer;
    F, S: string;
    FMode: Boolean;
    X: TStringList;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          S := Trim(Copy(S, 1, p - 1));
          if (StripDefaultDomain and (DefaultDomain <> '')) or
             ((not StripDefaultDomain) and (AnsiUppercase(DefaultDomain) = 'WORLD')) then
             // Always strip DefaultDomain for .world (backwards compatibilty)
          begin
            if AnsiUpperCase(Copy(S, Length(S) - Length(DefaultDomain), Length(DefaultDomain) + 1)) =
               AnsiUpperCase('.' + DefaultDomain) then
            begin
              S := Copy(S, 1, Length(S) - Length(DefaultDomain) - 1);
            end;
          end;
          ...
          begin
            ...
            // Add one or more aliases (comma separated)
            X := TStringList.Create;
            X.CommaText := S;
            for ix := 0 to X.Count - 1 do AliasList.Add(X[ix]);
            X.Free;
          end;
        end else begin
...
var l: Integer;
begin
  if OracleNLSLANG = '' then
  begin
    // Get NLS_LANG from the environment variable
    SetLength(OracleNLSLANG, 1000);
    l := GetEnvironmentVariable(PChar('NLS_LANG'), PChar(OracleNLSLANG), Length(OracleNLSLANG));
    SetLength(OracleNLSLANG, l);
    // Get NLS_LANG from the registry
    if OracleNLSLANG = '' then
      OracleNLSLANG := ReadRegString(HKEY_LOCAL_MACHINE, OracleHomeKey, 'NLS_LANG');
    OracleNLSLANG := AnsiUpperCase(OracleNLSLANG);
    OracleUTF8Client := Pos('UTF8', OracleNLSLANG) > 0;
  end;
  Result := OracleNLSLANG;
end;

function UTF8Client: Boolean;
begin
  if OracleNLSLANG = '' then NLS_LANG;
  Result := OracleUTF8Client;
end;
...
procedure BuildOracleHomeList;
{$IFNDEF LINUX}
var HomeCountString, HomeKey, HomeName, HomeDir: string;
...
{$IFNDEF LINUX}
  ...
  begin
    ...
    repeat
      ...
      begin
        HomeList.AddObject(HomeName, TObject(HomeIndex));
        dec(HomeCount);
      end;
      inc(HomeIndex);
  ...
  begin
    ...
    begin
      ...
      if (HomeDir <> '') and DirectoryExists(HomeDir + '\bin') then
        HomeList.AddObject(Copy(Names[i], 5, Length(Names[i])), TObject(-1));
    end;
  end;
  Names.Free;
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      OracleBinDir := (Length(HomeDir) > 4) and (AnsiUpperCase(Copy(HomeDir, Length(HomeDir) - 3, 4)) = '\BIN');
      if OracleBinDir then SetLength(HomeDir, Length(HomeDir) - 4);
    end else begin
      OracleBinDir := DirectoryExists(HomeDir + '\bin');
    end;
    Result := HomeDir;
  end;
  {$ENDIF}
...
{$ELSE}
  ...
  begin
    Retry:
    if OracleBinDir then AddToPath(Path, 'bin\') else AddToPath(Path, '');
    ...
    if (Result.Count = 0) and OracleBinDir then
    begin
      Path := OracleHomeDir;
      OracleBinDir := False;
      Goto Retry;
...
begin
  ...
  if GetEnvironmentVariable('_NS_ORA_INSTANT_CLIENT') = 'True' then //Add by XJG
    if DLL = '' then OCIDLL := GetEnvironmentVariable('_NS_OCIDLL'); //Add by XJG
  if OCIDLL <> '' then
  begin
...
procedure AdjustPath(Path: string);
const OriginalPath: string = '';
var S: string;
    p: Integer;
begin
  if not AllowAdjustPath then Exit;
  if OriginalPath = '' then OriginalPath := GetEnvironmentVariable('Path');
  S := OriginalPath;
  if Path <> '' then
  begin
    p := Pos(Uppercase(Path) + ';', Uppercase(S) + ';');
    if p > 0 then
    begin
      Delete(S, p, Length(Path) + 1);
    end;
    if S <> '' then S := Path + ';' + S else S := Path;
  end;
  {$IFDEF LINUX}
  SetEnv('Path', PChar(S), 1);
  {$ELSE}
  SetEnvironmentVariable('Path', PChar(S));
  {$ENDIF}
end;
{$ENDIF}
...
//    GetProc(HDLL, OCI80Detected, @OCICacheRefresh, 'OCICacheRefresh');
    ...
    GetProc(HDLL, OCI80Detected, @OCIStringSize, 'OCIStringSize');
    ...
    // TemporaryLOB
    GetProc(HDLL, OCI81Detected, @OCILobCreateTemporary, 'OCILobCreateTemporary');
    GetProc(HDLL, OCI81Detected, @OCILobFreeTemporary, 'OCILobFreeTemporary');
    GetProc(HDLL, OCI81Detected, @OCILobIsTemporary, 'OCILobIsTemporary');
    // Timestamp
    GetProc(HDLL, OCI81Detected, @OCIDateTimeToText, 'OCIDateTimeToText');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeFromText, 'OCIDateTimeFromText');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeGetDate, 'OCIDateTimeGetDate');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeGetTime, 'OCIDateTimeGetTime');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeConstruct, 'OCIDateTimeConstruct');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeAssign, 'OCIDateTimeAssign');
    GetProc(HDLL, OCI81Detected, @OCIDateTimeGetTimeZoneOffset, 'OCIDateTimeGetTimeZoneOffset');
    if not OCI81Detected then OCI81 := False;
    // New OCI 9.0 functions
    OCI90Detected := OCI81Detected;
    OCI90 := OCI90Detected;
    GetProc(HDLL, OCI90Detected, @OCIStmtFetch2, 'OCIStmtFetch2');
    if not OCI90Detected then OCI90 := False;
    // New OCI 9.2 functions
    OCI92Detected := OCI90Detected;
    OCI92 := OCI92Detected;
    GetProc(HDLL, OCI92Detected, @OCISessionPoolCreate, 'OCISessionPoolCreate');
    GetProc(HDLL, OCI92Detected, @OCISessionPoolDestroy, 'OCISessionPoolDestroy');
    GetProc(HDLL, OCI92Detected, @OCISessionGet, 'OCISessionGet');
    GetProc(HDLL, OCI92Detected, @OCISessionRelease, 'OCISessionRelease');
    GetProc(HDLL, OCI92Detected, @OCIStmtPrepare2, 'OCIStmtPrepare2');
    GetProc(HDLL, OCI92Detected, @OCIStmtRelease, 'OCIStmtRelease');
    if not OCI92Detected then OCI92 := False;
...
function OCIVersion: string;
begin
  if OCI92 then Result := 'version 9.2'
  else
    if OCI90 then Result := 'Version 9.0'
  else
...
begin
  ...
  try
    ...
    begin
      ...
      if MTSDLL = '' then
      begin
        MTSDLL := ExtractFilePath(OCIDLL) + 'oramts10.dll';
        if not FileExists(MTSDLL) then MTSDLL := ExtractFilePath(OCIDLL) + 'oramts.dll';
      end;
