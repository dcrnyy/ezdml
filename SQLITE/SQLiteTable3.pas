...
uses
  ...
  Windows,
  {$ENDIF}
  SQLite3, Classes, SysUtils, DB, kbmMemTable;
...
type
  ...
  public
    ...
    function GetTableA(const SQL: Ansistring): TSQLiteTable;
    ...
    procedure ExecSQLA(const SQL: Ansistring);
  ...
  private
    ...
    function GetFieldIndexA(FieldName: string): integer;
  public
    ...
    destructor Destroy; override;
    procedure ExportDB(const ADataSet: TkbmCustomMemTable; iCount: Integer);
    function FieldAsInteger(I: cardinal): int64;
    function FieldAsBlob(I: cardinal): TMemoryStream;
    function FieldAsBlobText(I: cardinal): string;
    function FieldIsNull(I: cardinal): boolean;
    function FieldAsString(I: cardinal): string;
    function FieldAsStringUtf8(I: cardinal): string;
    ...
    property FieldIndexA[FieldName: string]: integer read GetFieldIndexA;
...
begin
  ...
  Msg := nil;
  try
    utf8FileName := Utf8Encode(FileName);//UTF8String(FileName); //modified by huz
...
procedure TSQLiteDatabase.ExecSQLA(const SQL: Ansistring);
var
  S: String;
begin
  S:=Utf8Encode(Sql);
...
function TSQLiteDatabase.GetTableA(const SQL: Ansistring): TSQLiteTable;
var
  S: String;
begin
  S:=Utf8Encode(Sql);
...
procedure TSQLiteTable.ExportDB(const ADataSet: TkbmCustomMemTable; iCount: Integer);
  ...
  function STRNarrow(const Str: string): string;
  var
    I, L: Integer;
    S, T: string;
  begin
  ...
  procedure GetBlobFieldValue(Fld: TField; Idx: Integer);
  var
    Tmp, Src: TStream;
  begin
    Src := FieldAsBlob(Idx);
    Tmp := ADataSet.CreateBlobStream(Fld, bmReadWrite);
    try
      Tmp.CopyFrom(Src, 0);
    finally
      Tmp.Free;
...
begin
  ...
  begin
    Inc(C);
    if (iCount > 0) and (C > iCount) then
      Break;
    ADataSet.Append;
    for I := 0 to ADataSet.FieldCount - 1 do
      case ADataSet.Fields[I].DataType of
        ftBlob, ftGraphic:
          GetBlobFieldValue(ADataSet.Fields[I], I);
      else
        ADataSet.Fields[I].AsString := FieldAsStringUtf8(I);
      end;
    ADataSet.Post;
    Next;
  end;
...
function TSQLiteTable.GetFieldIndexA(FieldName: string): integer;
var
  S: String;
begin
  S := Utf8Encode(FieldName);
  Result := GetFieldIndex(S);
end;
...
function TSqliteTable.FieldAsStringUtf8(I: cardinal): string;
begin
