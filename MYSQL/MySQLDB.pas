(******************************************************************
**
** TMySQLDataset component version 0.8
**
...
type
  ...
  public
    ...
    property MysqlCon: PMYSQL read FMYSQL write FMYSQL;
  published
    property Active;
...
procedure TMySQLDataset.DataConvert(Field: TField; Source, Dest: Pointer;
  ToNative: Boolean);
begin
...
begin
  ...
  begin
    ...
    if Field.FieldName = mysql_field_name(fld) then
    begin
      Move(CurBuf^, PChar(Buffer)^, MySQLDataSize(mysql_field_type(fld), mysql_field_length(fld)));
      ...
      Inc(CurBuf, MySQLDataSize(mysql_field_type(fld), mysql_field_length(fld)));
  end;
end;
...
procedure TMySQLDataset.InternalAddRecord(Buffer: Pointer;
  Append: Boolean);
begin
  raise Exception.Create('Not supported for mysql');
end;
...
begin
  FCurrentRecord := Integer(Bookmark);
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if MySQLFieldToFieldType(mysql_field_type(field), mysql_field_length(field), DFT, DFS) then
          TFieldDef.Create(FieldDefs, mysql_field_name(field), DFT, DFS, False, I + 1);
      end;
    finally
      if WasClosed then
      begin
...
begin
  ...
  begin
    field := mysql_fetch_field_direct(FMYSQLRES, I);
    FRecordSize := FRecordSize + MySQLDataSize(mysql_field_type(field), mysql_field_length(field));
  end;
...
begin
  ...
  begin
    field := mysql_fetch_field_direct(FMYSQLRES, I);
    CT := MySQLWriteFieldData(mysql_field_type(field), mysql_field_length(field), row^[I], Buffer);
    Inc(Buffer, CT);
  end;
end;
...
begin
  if FMYSQL = nil then
    DatabaseError('Error no connection found');
end;
...
procedure Register;
begin
  RegisterComponents('Data Access', [TMySQLDataset]);
end;
